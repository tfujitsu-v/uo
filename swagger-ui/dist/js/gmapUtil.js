...
GMapUtil.enableSaveInitialView = function(map, localStorageKey) {
    if (localStorageKey == undefined) {
        localStorageKey = 'savedView';
    }
    // 保存されていたら復元
    var savedView = localStorage.getItem(localStorageKey);
    if (savedView) {
        try {
    ...
    function onViewChanged() {
        ...
        localStorage.setItem(localStorageKey, JSON.stringify(savedView));
    };
...
GMapUtil.enableSetOriginDestination = function(map, callback) {
    ...
    window.setAs = function(as, lat, lng) {
        if (as == 'origin') {
            ...
            window.destinationMarker.setTitle('目的地: ' + lat + ',' + lng);
        }
        setAsWindow.close();
        if (callback) {
            callback(as, lat, lng);
    ...
    map.addListener('click', function(e) {
        ...
        content += '<a href="" onclick="setAs(\'destination\', ' + lat + ',' + lng + '); return false;" class="setAs">目的地に指定</a>';
